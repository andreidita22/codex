...
- `thread/refresh/start` — trigger between-turn continuity artifact refresh (`/refresh` equivalent) for a thread; returns `{}` immediately while progress streams through standard turn/item notifications.
- `thread/prune/start` — trigger deterministic between-turn compact-context pruning (`/prune` equivalent) for a thread; returns `{}` immediately while progress streams through standard turn/item notifications.
