...
#[tokio::test]
async fn loads_continuation_bridge_prompt_from_config() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let cfg = ConfigToml {
...
#[tokio::test]
async fn loads_continuation_bridge_prompt_from_file() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let workspace = codex_home.path().join("workspace");
    std::fs::create_dir_all(&workspace)?;
...
#[tokio::test]
async fn continuation_bridge_prompt_prefers_inline_over_file() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let workspace = codex_home.path().join("workspace");
    std::fs::create_dir_all(&workspace)?;
...
#[tokio::test]
async fn loads_continuation_bridge_variant_model_and_reasoning() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let cfg = ConfigToml {
...
#[tokio::test]
async fn loads_governance_path_variant() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let cfg = ConfigToml {
...
#[tokio::test]
async fn loads_compaction_engine_and_context_maintenance_settings() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let cfg = ConfigToml {
