...
impl Session {
    ...
    async fn build_settings_update_items(
        ...
        if let Some(collab_instructions) = CollaborationModeInstructions::from_collaboration_mode(
            &current_context.collaboration_mode,
        ) {
            full_role_sections.push(collab_instructions.render());
        }
        ...
        if current_context.realtime_active {
            ...
            full_runtime_sections.push(realtime_instructions);
        }
        ...
        if current_context.apps_enabled() {
            ...
            {
                full_runtime_sections.push(apps_instructions.render());
            }
        }
        ...
        if let Some(available_skills) = build_available_skills(
            &implicit_skills,
            default_skill_metadata_budget(current_context.model_info.context_window),
            SkillRenderSideEffects::None,
        ) {
            full_runtime_sections
                .push(AvailableSkillsInstructions::from(available_skills).render());
        }
        ...
        {
            full_runtime_sections.push(plugin_instructions.render());
        }
