    ...
    if let Err(err) = live_thread.flush().await {
        sess.send_event_raw(
            Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    message: format!(
                        "failed to flush thread persistence for rollback replay: {err}"
                    ),
                    codex_error_info: Some(CodexErrorInfo::ThreadRollbackFailed),
                }),
            },
        ...
        Err(err) => {
            sess.send_event_raw(
                Event {
                    ...
                    msg: EventMsg::Error(ErrorEvent {
                        message: format!(
                            "failed to load thread history for rollback replay: {err}"
                        ),
                        codex_error_info: Some(CodexErrorInfo::ThreadRollbackFailed),
                    }),
                },
    ...
    while let Ok(sub) = rx_sub.recv().await {
        ...
        let should_exit = async {
            match sub.op.clone() {
                ...
                Op::RefreshContext => {
                    refresh_context(&sess, sub.id.clone()).await;
                    false
                }
                Op::PruneContext => {
                    prune_context(&sess, sub.id.clone()).await;
                    false
                }
                Op::DropMemories => {
