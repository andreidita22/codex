...
"#
    ...
    let second_request = second_completion.single_request();
    let second_items = function_tool_output_items(&second_request, "call-2");
    let completion_items = if second_items.len() == 1 {
        assert_regex_match(
            concat!(
                r"(?s)\A",
                r"Script running with cell ID \d+\nWall time \d+\.\d seconds\nOutput:\n\z"
        ...
        test.submit_turn("wait again for completion").await?;

        function_tool_output_items(&third_completion.single_request(), "call-3")
    } else {
        second_items
    };
...
\z
"#;
    assert_regex_match(expected_pattern, text_item(&completion_items, /*index*/ 1));
